...
const Header = () => {
    ...
    const CustomizedLinkButton = ({ name, path, displayName }: { name: string, path: string, displayName: string }) => {
    ...
    return (
        <ThemeProvider theme={customMuiTheme}>
            <div className='headerContainer'>
                ...
                >
                    <CustomizedLinkButton displayName={width ? "About me" : "About"} name={"About me"} path='/home' />
                    <CustomizedLinkButton displayName={width ? "Skills and Certifications" : "Skills"} name={"Skills and Certifications"} path='/skills' />
                    <CustomizedLinkButton displayName={"Achievements"} name={"Achievements"} path='/achievements' />
                </Stack>
                ...
                <Stack direction={width ? 'row' : 'column'} display={"flex"} justifyContent={width ? "flex-end" : "center"} alignItems={"center"} spacing={2} mb={width ? 0 : 5}>

                    {/* Resume download section */}
                    <Link href="/Vishwakanth Resume 2024.pdf" download="Vishwakanth Resume 2024.pdf">
                        <CustomActionButton Icon={() => <FileCopyOutlined style={{ marginTop: -2 }} />} title='Download My Resume' bgColor={appColors.download_bg} />
                    </Link>

                    {/* Connect to linkedin section */}
                    <Link href={"https://www.linkedin.com/in/vishwakanth-s-7a17342b0"}>
                        <CustomActionButton Icon={() => <LinkedIn style={{ marginTop: -5 }} />} title='Connect with me' bgColor={appColors.connect_bg} />
                    </Link>
